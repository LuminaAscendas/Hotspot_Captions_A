...
var direction_text='<span id="dir_span">Directions:</span> Click on each question mark to progress through the graphic. You may close the caption by clicking on the “X” or by clicking on the next question mark.'
...
//			},500);
		...
		$("#begin_container,#image_container").mouseenter(function(){
			$('#begin_container,#image_container').attr('title',"Sustainable development: three essential factors");
		});
...
//		},100)
		...
		if( /Android|webOS|iPhone|iPad|iPod|Opera Mini/i.test(navigator.userAgent) ) {
			setTimeout(function(){	
			   ...
			   //	
			    if (/MSIE 10/i.test(navigator.userAgent) || /MSIE 9/i.test(navigator.userAgent) || /rv:11.0/i.test(navigator.userAgent) ||/Edge\/\d./i.test(navigator.userAgent)) {
					$('#label_'+curr_id).focus().removeAttr('role');
				}else{
//				setTimeout(function(){
...
function set_tab(){
	if(begin_entered==false){
		...
		$('.label_class').addClass('tab_index');
